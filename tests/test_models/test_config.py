...
class TestServerConfig:
    ...
    def test_default_values(self):
        ...
        assert config.admin_group == "chimera-admin"
    ...
    def test_custom_values(self):
        ...
        config = ServerConfig(
            ...
            log_level="DEBUG",
            admin_group="container-admins",
        )

        assert config.reconciliation_interval == 60
        assert config.log_level == "DEBUG"
        assert config.admin_group == "container-admins"
    ...
    def test_interval_validation(self):
        ...
        # Minimum value
        with pytest.raises(ValidationError) as exc_info:
            ServerConfig(reconciliation_interval=4)  # Less than 5

        assert "reconciliation_interval" in str(exc_info.value)

    def test_remote_requires_tls_configuration(self):
        """Enabling a bind address without TLS material is invalid."""
...
class TestChimeraConfig:
    ...
    def test_default_config(self):
        ...
        # Check defaults
        assert config.server.admin_group == "chimera-admin"
    ...
    def test_extra_fields_ignored(self):
        ...
        config = ChimeraConfig(server=ServerConfig(), extra_field="ignored")
