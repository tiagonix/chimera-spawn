...
from chimera.models.image import CustomFileSpec, ImageProductPolicy, ImageSourceSpec
...
async def test_provision_rootfs_refuses_a_running_container(tmp_path):
    ...
    spec = ContainerSpec(name="demo", image="ubuntu", image_source="ubuntu")
    spec._effective_image = effective_from_source(
        ...
        ImageProductPolicy(custom_files=[CustomFileSpec(path="etc/example", ensure="absent")]),
    )
...
@pytest.mark.asyncio
async def test_clone_uses_effective_local_image_name(tmp_path):
    """Containers clone the derived local cache, not the typed product key."""
    provider = ContainerProvider()
    provider.machines_dir = tmp_path / "machines"
    provider.nspawn_dir = tmp_path / "nspawn"
    provider.system_dir = tmp_path / "system"
    ...
    provider.inspect_materialization = AsyncMock(return_value=(ProviderStatus.ABSENT, None))
...
async def test_preflight_rejects_online_name_without_local_artifacts(tmp_path):
    ...
    spec = ContainerSpec(name="foreign", image="ubuntu", image_source="ubuntu")
...
@pytest.mark.asyncio
async def test_generated_cache_image_is_owned_unrelated_image_is_unmanaged(tmp_path):
    """Generated Chimera image caches are owned; unrelated host images stay unmanaged."""
    from chimera.images.identity import cache_image_name
    ...
    provider.nspawn_dir.mkdir()
    provider.system_dir.mkdir()
    cache_name = cache_image_name("ubuntu", "com.ubuntu.cloud:server:24.04:amd64")
    (provider.machines_dir / cache_name).mkdir()
    (provider.machines_dir / "foreign-image").mkdir()
    (provider.machines_dir / "chimera-other").mkdir()
    (provider.machines_dir / "chimera-src-random-junk").mkdir()
    provider.systemd_dbus = AsyncMock()
    provider.systemd_dbus.list_machines = AsyncMock(return_value=[])

    resources = await provider.list_unmanaged_host_resources(set())
    assert cache_name in resources["image_caches"]
    assert cache_name not in resources["storage_entries"]
    assert "foreign-image" in resources["storage_entries"]
    assert "chimera-other" in resources["storage_entries"]
    assert "chimera-src-random-junk" in resources["storage_entries"]
    assert "chimera-src-random-junk" not in resources["image_caches"]
